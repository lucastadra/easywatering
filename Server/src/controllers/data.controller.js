...
exports.GetByESP = async (req, res) => {
    try {
      ...
      if (esp.user_id != user.id) {
        return res.status(401).send({ message: "User not authorized to view ESP Data." });
      }

      const data = await ESPData.findAll({where: {
        esp_id: esp.id
      }})
...
exports.GetByType = async (req, res) => {
    try {
      ...
      if (req.params.typeId && !isNaN(parseInt(req.params.typeId, 10))) {
